  ...
  return (
    <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-[11px] text-slate-300">
      <span className="flex items-center gap-1">
        ...
        <span className="ml-1">Anomalía (alta → baja)</span>
      </span>
      <span className="flex items-center gap-1.5"><i className="inline-block h-0.5 w-4 rounded bg-[#38bdf8]" /> Tasa global</span>
      <span className="flex items-center gap-1.5"><i className="inline-block w-4 border-t-2 border-dashed border-[#fb923c]" /> Nivel 5</span>
      <span className="flex items-center gap-1.5"><i className="size-2.5 rounded-full bg-[#fbbf24]" /> Sismo (M)</span>
    </div>
  )
}
  ...
  return (
    <section className="flex h-[56vh] min-h-[500px] shrink-0 flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] p-4 backdrop-blur-xl">
      <header className="mb-1 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-base font-semibold text-white">Energía entrante por anomalía magnética</h1>
          <p className="text-xs text-slate-400">Método Cordaro · ángulo Luna–Tierra–Sol frente al tiempo · <span className="font-mono text-cyan-300">{date.toISOString().slice(0, 10)}</span></p>
        </div>
        <Legend />
      </header>
      <div className="min-h-0 flex-1">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={visible} margin={{ top: 12, right: 20, bottom: 28, left: 60 }} barGap={0} barCategoryGap={0}>
            ...
            <XAxis dataKey="time" tick={{ fontSize: 10, fill: '#94a3b8' }} interval={7} tickLine={{ stroke: '#475569' }} axisLine={{ stroke: '#475569' }} label={{ value: 'Tiempo (UTC)', position: 'insideBottomRight', offset: -14, fontSize: 10, fill: '#94a3b8' }} />
            <YAxis domain={[0, 10]} ticks={[0, 2, 4, 5, 6, 8, 10]} tick={{ fontSize: 10, fill: '#94a3b8' }} label={{ value: 'Probabilidad sísmica relativa', angle: -90, position: 'insideLeft', offset: -40, fontSize: 10, fill: '#f87171' }} />
            ...
            <ReferenceLine y={5} stroke="#fb923c" strokeWidth={1} strokeDasharray="6 4" label={{ value: 'Nivel 5', position: 'right', fill: '#fdba74', fontSize: 10 }} />
