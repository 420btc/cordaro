  ...
  return (
    <MapContainer center={[8, 4]} zoom={1.5} minZoom={1} maxZoom={5} scrollWheelZoom={false} worldCopyJump className="h-full w-full" attributionControl={false}>
      ...
      <div className="pointer-events-none absolute left-4 top-4 z-[500] rounded-xl border border-white/10 bg-slate-950/70 px-3 py-2 backdrop-blur">
        <p className="text-[11px] font-bold uppercase tracking-wider text-cyan-300">{t('map.title')}</p>
        <p className="font-mono text-[10px] text-slate-400">{t('map.subtitle')}</p>
      </div>
      ...
      <div className="pointer-events-none absolute right-3 top-3 z-[500] w-48 rounded-xl border border-white/10 bg-slate-950/75 p-3 text-[10px] leading-relaxed text-slate-300 backdrop-blur">
        <p className="mb-1.5 font-bold uppercase tracking-wider text-slate-400">{t('map.legend')}</p>
        <p><i className="mr-1.5 inline-block size-2 rounded-full bg-amber-300" /> {t('map.moon')}</p>
        <p><i className="mr-1.5 inline-block size-2 rounded-full bg-orange-400" /> {t('map.sun')}</p>
        <p><i className="mr-1.5 inline-block size-2 rounded-full bg-blue-500" /> {t('map.antipode')}</p>
        <p><i className="mr-1.5 inline-block h-0.5 w-4 rounded bg-red-500" /> {t('map.plateBoundary')}</p>
        <p><i className="mr-1.5 inline-block w-4 border-t-2 border-dashed border-red-400" /> {t('map.moonPath')}</p>
        <p><i className="mr-1.5 inline-block w-4 border-t-2 border-dashed border-amber-400" /> {t('map.sunPath')}</p>
        <p><i className="mr-1.5 inline-block text-red-500">▲</i> {t('map.station')}</p>
        <p><i className="mr-1.5 inline-block size-2 rounded-full border-2 border-white bg-rose-500" /> {t('map.crossingMoon')}</p>
        <p><i className="mr-1.5 inline-block size-2 rounded-full border-2 border-white bg-blue-500" /> {t('map.crossingAntipode')}</p>
      </div>
