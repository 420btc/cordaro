...
import { fetchRegionalQuakes, magColor, regionForLocation, REGIONS, type RegionId, type RegionalQuake } from '@/lib/regionalSeismicity'
import { haversineKm } from '@/lib/earthquakes'
import { useI18n } from '@/lib/i18n'

const RegionalMap = dynamic(() => import('./RegionalSeismicityMapInner').then((m) => m.RegionalSeismicityMapInner), {
  ssr: false,
...
export function RegionalSeismicityPanel() {
  const { t } = useI18n()
  const [region, setRegion] = useState<RegionId | null>(null)
  const [quakes, setQuakes] = useState<RegionalQuake[] | null>(null)
  ...
  const requestLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        setLocation(coords)
        const detected = regionForLocation(coords.latitude, coords.longitude)
    ...
    <section className="rounded-md border border-[#29313b] bg-[#151a21] p-4 shadow-sm">
      <header className="mb-3 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          {location && <span className="rounded-full border border-[#5b8db8]/50 bg-[#5b8db8]/10 px-2.5 py-1 font-mono text-[10px] font-semibold text-[#5b8db8]">{t('ign.locationOn')}</span>}
          <select
            value={region ?? ''}
            onChange={(e) => setRegion((e.target.value || null) as RegionId | null)}
            className="h-8 rounded border border-[#29313b] bg-[#1c232b] px-2 font-mono text-xs text-[#e7eaee]"
          >
            <option value="">{t('ign.select')}</option>
            {REGIONS.map((r) => <option key={r.id} value={r.id}>{t(`region.${r.id}` as any)}</option>)}
          </select>
      ...
      {!region ? (
        <div className="flex h-64 items-center justify-center rounded-md border border-[#29313b] bg-[#0e1116] px-4 text-center font-mono text-xs text-[#8b94a0]">{t('ign.noRegion')}</div>
      ) : (
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
          <div className="h-[420px] overflow-hidden rounded-md border border-[#29313b] bg-[#0e1116]">
            ...
            {!loading && !error && quakes && <RegionalMap quakes={quakes} region={region} userLocation={location} />}
          </div>
              ...
              <ul className="divide-y divide-[#29313b]">
                {visible.map((quake) => (
                  <li key={quake.id} className="flex items-center gap-3 px-3 py-2">
                    <span className="flex min-w-[2.5rem] items-center justify-center rounded px-1.5 py-0.5 font-mono text-[11px] font-bold" style={{ color: magColor(quake.magnitude), background: `${magColor(quake.magnitude)}18` }}>M{quake.magnitude.toFixed(1)}</span>
      ...
      <footer className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-[#29313b] pt-2 font-mono text-[10px] text-[#8b94a0]">
        <span className="flex flex-wrap items-center gap-2">
          {[4, 3, 2, 1].map((mag) => (
            <span key={mag} className="flex items-center gap-1"><i className="size-2 rounded-full" style={{ background: magColor(mag) }} /> ≥{mag}</span>
          ))}
        </span>
