...
import { formatUtc, generateDemoEarthquakes } from './types'

export async function fetchEarthquakes(day: Date): Promise<Earthquake[]> {
  const start = day.toISOString(); const end = new Date(day.getTime() + 86400000).toISOString()
  const url = `https://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson&starttime=${start}&endtime=${end}&minmagnitude=3&orderby=time`
  try {
  ...
  } catch { return generateDemoEarthquakes(day) }
}
